...
namespace App\Http\Controllers\Account\My\Surveys;
...
class EedSurveyController extends Controller
{
    ...
    public function index($id)
    {
        ...
        return view('accounts.my.surveys.eed-surveys.index')
            ->with('surveys', $surveys)
            ->with('account', $id)
            ->with('total', $surveys->total())
            ->with('indexUrl', route('my.account.eed-surveys.index', $id));
    }

    public function my()
    {

        $surveys = EedSurvey::with('account','callStatus')->where('createdBy', '=', session('userName'))->orderBy('createdOn', 'desc')->paginate(20);

        return view('surveys.my.eed-surveys.index')
            ->with('surveys', $surveys)
            ->with('total', $surveys->total())
            ->with('indexUrl', route('my.eed-surveys.index'));
    }
...
//dd($EidQ3);
        ...
        return view('accounts.my.surveys.eed-surveys.create')
            ->with('EidQ1', $EidQ1)
            ->with('EidQ2', $EidQ2)
            ->with('EidQ3', $EidQ3)
