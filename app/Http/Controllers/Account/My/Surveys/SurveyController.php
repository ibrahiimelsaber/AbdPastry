...
class SurveyController extends Controller
{
    ...
    {
        $surveys = EedSurvey::with('account','callStatus')->where('AccountId', '=', $id)->orderBy('createdBy', session('userName'))->paginate(20);
        return view('accounts.my.norm-surveys.index')
            ->with('surveys', $surveys)
            ->with('total', $surveys->total())
            ->with('indexUrl', route('my.surveys.index'));
    }
